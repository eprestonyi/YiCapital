...
(function (root, factory) {
  ...
  function analyze(workbookSheets, opts = {}) {
    ...
    // 基準（如提供 Benchmark sheet）
    const benchmarks = {};
    if (benchPrices) {
      Object.entries(benchPrices).forEach(([name, prices]) => {
        const br = priceToReturns(prices);
        const al = align(rets, br);
        if (al.a.length >= 5) benchmarks[name] = { aligned: al, metrics: calcMetrics(al.a, al.b), benchMetrics: calcMetrics(al.b, null), curve: equityCurve(al.dates.map((d, i) => ({ date: d, ret: al.b[i] }))) };
      });
    }

    const metrics = calcMetrics(rp, null);
    const primaryBM = Object.keys(benchmarks)[0] || null;
    if (primaryBM) Object.assign(metrics, {
      alpha: benchmarks[primaryBM].metrics.alpha, beta: benchmarks[primaryBM].metrics.beta,
      r2: benchmarks[primaryBM].metrics.r2, infoRatio: benchmarks[primaryBM].metrics.infoRatio,
      trackingErr: benchmarks[primaryBM].metrics.trackingErr, treynor: benchmarks[primaryBM].metrics.treynor,
    });

    return {
      asOf: asOf || navRows[navRows.length - 1].date,
      navRows, assets, rets, rp, metrics, benchmarks, primaryBM,
  ...
  return {
    analyze, parseNavSheet, parseAssetSheet, parseBenchmarkSheet,
