...
(function () {
  ...
  const D = {
    ...
    /* ── 首頁 ── */
    'hero.s1k':['Research ｜ 首次覆蓋','Research ｜ 首次覆盖','Research | Initiation'],
    'hero.s1t':['騰訊控股（0700.HK）：流量帝國的價值重估時刻','腾讯控股（0700.HK）：流量帝国的价值重估时刻','Tencent (0700.HK): The Revaluation Moment of a Traffic Empire'],
    'hero.s2k':['Portfolio ｜ Yi Capital HK','Portfolio ｜ Yi Capital HK','Portfolio | Yi Capital HK'],
    'hero.s2t':['港股週期均值回歸策略','港股周期均值回归策略','HK Cyclical Mean-Reversion Strategy'],
    'hero.s3k':['Forum ｜ 研報庫','Forum ｜ 研报库','Forum | Research Library'],
    ...
    'home.hk.k':['Featured ｜ 港股','Featured ｜ 港股','Featured | Hong Kong'],
    ...
    'ch.dd':['回撤 Drawdown','回撤 Drawdown','Drawdown'],
  ...
  window.YCI = { lang, t, f, lbl, set: l => { localStorage.setItem('yc-lang', l); location.reload(); } };
  ...
  function mount() {
    ...
    if (lang !== 'tw') {
      ...
      if (/insights|forum/.test(p) || document.getElementById('home-reports')) {
        const host = document.querySelector('#report-list, #posts-list, #home-reports');
