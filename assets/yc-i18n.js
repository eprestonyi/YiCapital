...
(function () {
  ...
  function pageTarget(l) {
    ...
    // posts / admin 等未三語化頁面 → 跳對應語言首頁族
    const TRIPLED = ['index.html','portfolios.html','fund-us.html','insights.html','forum.html','filings.html','about.html','login.html','terms.html'];
    const TRIPLED_POSTS = ['tencent-0700-ch12.html','yicapital-risk-report.html'];
    if (parts.indexOf('posts') >= 0) {
      file = TRIPLED_POSTS.indexOf(file) >= 0 ? 'posts/' + file : 'index.html';
    } else if (TRIPLED.indexOf(file) < 0) file = 'index.html';
    return (l === 'tw' ? '/' : '/' + l + '/') + file + location.hash;
  }
