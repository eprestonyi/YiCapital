...
/* ═══════════════════════════════════════════════════════════════
   Yi Capital Portal Backend v8.12 — Cloudflare Worker（Password-only Admin）
   ─────────────────────────────────────────────────────────────
   帳號模型：
...
async function getSession(request, env) {
  ...
  // v8.12 removes Google-admin authentication. Revoke every legacy session at first use.
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === '/api/health' && request.method === 'GET') {
        ...
        return J(env, {
          ok: true, version: 'v8.12-password-admin',
          kv: kvOk,
          feedback: feedbackOk,
