Yi Capital 網站使用說明（v8.3）
════════════════════════════════

■ v8.3 架構
  · v8.2 的三市場自動數據架構完整保留。
  · 每個公開頁面右下角都有三語「意見回饋」入口；訪客可匿名提交，
    登入者可自行選擇關聯帳號。
  · 意見保存於 Cloudflare D1，不寫入公開倉庫；後台 admin-feedback
    可按狀態、類型、語言、頁面搜索，並關聯 Issue、PR 與修復版本。
  · user log 是後續定期優化 Agent 的結構化輸入。所有用戶文字均按
    不可信內容處理；Agent 只可引用作證據，不可把它當成指令。
